...
if ~isempty(overlay) && cell_tog
    ...
    if cell_idx==0
        ...
        for tmp_idx=1:length(cell_signals)
            ...
            % NOTE - IF-ELSE added 05/21 for adjusted TMP_CELL definition
            if ~isempty(curr_cell_logic)
            ...
            color_ind = mod(tmp_idx,7);
            if color_ind==0
                color_ind = 7;
            end
            curr_color = co(color_ind,:);
            ...
            if excl_tog
                ...
                assignin('base','tmp_cell_logic',tmp_cell_logic);
                % logically_excluded = ~feature_incl;
                % logically_excluded(curr_cell_logic==0) = 0;
                if ~isempty(tmp_cell_logic)
                    excl_coords = tmp_obj_coords(logical(tmp_cell_logic),:);
                else
                    excl_coords = [];
                end
