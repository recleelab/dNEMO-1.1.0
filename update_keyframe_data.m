...
if contains(kf_tokens{1},'+')
    ...
    switch kf_tokens{2}
        case 'CELL'
            ...
            user_temporal_bound = curr_cell.user_temporal_bound;
            frame_strings = cat(1,frame_strings,strcat('    >Frame',num2str(user_temporal_bound(1)),'_set_cell_start'));
            
            adj_mat = curr_cell.pseudoAdjMatrix;
            % [~,frames,~] = unique(adj_mat(1,:));
            frames = find(adj_mat(2,:)>0);
            
            for frame_idx=1:length(frames)
                frame_strings = cat(1,frame_strings,strcat('    >Frame',num2str(frames(frame_idx)),'_cell_modified'));
            end
            
            frame_strings = cat(1,frame_strings,strcat('    >Frame',num2str(user_temporal_bound(2)),'_set_cell_stop'));
...
if contains(curr_str,'[+]') || contains(curr_str,'[-]')
    ...
    switch str_tokens{2}
        case 'CELL'
            ...
            range_cell = uimenu('Parent',cell_edit_menu,'Label','Remove Seg. Keyframes',...
                                'callback',{@polygon_set_range, APP});
            quick_redraw_cell = uimenu('Parent',cell_edit_menu,'Label','Quick Seg. Redraw',...
                                       'callback',{@polygon_quick_frame_resegmentation, APP});
            reset_cell_user_bounds = uimenu('Parent', cell_edit_menu, 'Label', 'Reset Cell Start/Stop',...
                                            'callback', {@polygon_set_temporal_bound, APP});
            hand.UIContextMenu = cell_edit_menu;
            % hand.UIContextMenu.Visible = 'on';
            % hand.UIContextMenu.Position = [1000 500];
...
if contains(curr_str,'>')
    ...
    if contains(curr_str,'cell_modified')
        ...
        delete_cell = uimenu('Parent',cell_edit_menu,'Label','Delete Cell Keyframe',...
                            'callback',{@polygon_remove_kftmp,APP});
        delete_cell_FULL = uimenu('Parent',cell_edit_menu,'Label','Delete Cell',...
                                  'callback',{@polygon_remove,APP});
        quick_redraw_cell = uimenu('Parent',cell_edit_menu,'Label','Quick Seg. Redraw',...
                                   'callback',{@polygon_quick_frame_resegmentation, APP});
    ...
    if contains(curr_str,{'MEAN','MEDIAN','SUM','SIZE','MAX'})
        ...
        % hand.UIContextMenu.Visible = 'on';
        % hand.UIContextMenu.Position = [1075 455];
