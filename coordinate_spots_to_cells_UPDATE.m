function [] = coordinate_spots_to_cells_UPDATE(APP, cell_no)
%% <placeholder>
%
...
if size(cell_signals,1) < size(polygon_list,1) && ~isempty(cell_signals)
    [cell_signals_in_polygon] = cell_signal_assignment_UPDATE(spot_detect, polygon_list{size(polygon_list,1)});
    cell_signals = cat(1,cell_signals,{cell_signals_in_polygon});
%{
elseif isempty(cell_signals) && ~isempty(polygon_list)
...
%}
    ...
    %{
    cell_signals = {};
    for cell_idx=cell_no:size(polygon_list)
        [cell_signals_in_polygon] = cell_signal_assignment_UPDATE(spot_detect, polygon_list{cell_idx});
        cell_signals = cat(1,cell_signals, {cell_signals_in_polygon});
    end
    %}
    [cell_signals_in_polygon] = cell_signal_assignment_UPDATE(spot_detect, polygon_list{cell_no});
    cell_signals{cell_no} = cell_signals_in_polygon;
end

if size(cell_signals,1) > 1
    updated_cell_signals = fcfs_UPDATE(cell_signals);
...
function [cell_signals] = cell_signal_assignment_UPDATE(spot_detect_obj, tmp_cell_obj)
%% <placeholder>
%

% assignin('base','spot_detect_obj',spot_detect_obj);
% assignin('base','tmp_cell_obj',tmp_cell_obj);
...
for n=1:size(polygons,2)
    ...
    % modified below if to include ~isnan(curr_polygon)
    if ~isempty(curr_spotInfo) && max(any(~isnan(curr_polygon)))
...
function [cell_signals] = fcfs_UPDATE(cell_signals)
%% <placeholder>
%
