...
file_menu_avi = uimenu(file_menu,'label','Save as AVI');
...
image_menu_reset = uimenu(image_menu,'label','Set Image Dims.');
image_menu_two_dim = uimenu(image_menu,'label','Make Image 2D');
...
% AUTOMATED SEGMENTATION SETTINGS
auto_seg_menu = uimenu(APP.MAIN,'label','Cell Masks');
auto_seg_menu_cellpose = uimenu(auto_seg_menu, 'label', 'Run Cellpose');
auto_seg_menu_settings = uimenu(auto_seg_menu, 'label', 'Adjust Import Settings');
auto_seg_menu_mask = uimenu(auto_seg_menu, 'label', 'Import Cell Mask TIFF');
auto_seg_menu_csv = uimenu(auto_seg_menu, 'label', 'Import Cell Mask CSV/XLS');
...
set(file_menu_avi,'callback',{@save_as_avi,APP});
...
set(image_menu_reset, 'callback', {@set_image_dims, APP});
set(image_menu_two_dim, 'callback', {@set_image_two_dim, APP});
% set([image_menu_mask, image_menu_csv],'callback',{@cell_mask_handler, APP});
set(auto_seg_menu_cellpose,'callback',{@run_cellpose_from_dnemo,APP});
set(auto_seg_menu_settings,'callback',{@confirm_mask_import_settings,APP});
set([auto_seg_menu_mask, auto_seg_menu_csv], 'callback', {@cell_mask_handler, APP});
